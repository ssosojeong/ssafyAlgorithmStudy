...
public class P15710_메모리초과 {
	static int[] memo;//10^9+7로 나눈 나머지들을 저장할꺼기 때문에 int 형이여도 된다.
	static long p = (long)Math.pow(2, 31);
	static int q = (int)Math.pow(10, 7)+7;
	...
	private static int findMod(int n) {
		...
		if(n%2==0) {//짝수
			...
			int temp = memo[n/2]; 
			if(temp!=0) {
				int result = memo[n] = (int)(((long)(temp*temp))%q);
				return result;
			}//if
			else {
				memo[n/2] = temp = findMod(n/2);
				int result = memo[n] = (int)(((long)(temp*temp))%q);
				return result;
			}//else
		} else {//홀수
			int temp = memo[(n-1)/2];
			if(temp!=0) {
				int result = memo[n] = (int)((( ((long)(temp*temp))%q ) * r) % q);
				return result;
			}
			else {
				memo[(n-1)/2] = temp = findMod((n-1)/2);
				int result = memo[n] = (int)((( ((long)(temp*temp))%q ) * r) % q);
				return result;
			}
		}
